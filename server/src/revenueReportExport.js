...
function setupPrint(ws, titleRows = '1:1') {
  ...
  ws.headerFooter = { oddFooter: '&LApp Report New&RTrang &P/&N' };
}
...
async function excelBuffer(report) {
  const wb = new ExcelJS.Workbook(); wb.creator = 'App Report New'; wb.created = new Date();
...
function pdfBuffer(report) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'A4', layout: 'landscape', margins: { top: 34, bottom: 34, left: 36, right: 36 }, bufferPages: true, info: { Title: `Báo cáo doanh thu ${report.ky}`, Author: 'App Report New' } });
    ...
    title(`BÁO CÁO DOANH THU BÁN HÀNG — ${report.kys.join(', ') || report.ky}`, `Cập nhật ${dateText(report.updatedAt)} · App Report New`);
    ...
    const range = doc.bufferedPageRange(); for (let i = range.start; i < range.start + range.count; i++) { doc.switchToPage(i); doc.font(F).fontSize(8).fillColor('#94a3b8').text(`App Report New · Trang ${i + 1}/${range.count}`, 36, doc.page.height - doc.page.margins.bottom - 12, { width: pageW, align: 'right', lineBreak: false }); }
    doc.end();
  });
}
...
async function pptxBuffer(report) {
  const pptx = new PptxGenJS(); pptx.layout = 'LAYOUT_WIDE'; pptx.author = 'App Report New'; pptx.subject = `Báo cáo doanh thu ${report.ky}`; pptx.title = `Báo cáo doanh thu ${report.ky}`; pptx.company = 'DONAPHARM'; pptx.lang = 'vi-VN'; pptx.theme = { headFontFace: 'Arial', bodyFontFace: 'Arial', lang: 'vi-VN' };
