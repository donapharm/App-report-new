...
function normalizeParameters(raw = {}) {
  ...
  const parameters = {
    ...
    lowerRatePct: finiteInRange(raw.lowerRatePct, 'Tỷ lệ phạt bậc thấp', 0, 100),
    upperRatePct: finiteInRange(raw.upperRatePct, 'Tỷ lệ phạt bậc trên', 0, 100),
    bottomDropC45: raw.bottomDropC45 === true,
    bottomRatePct: finiteInRange(raw.bottomRatePct, 'Tỷ lệ phạt bậc đáy', 0, 100),
...
function createPolicyStore({ policyFile = POLICY_FILE, auditFile = AUDIT_FILE, seedConfig = null, now = () => new Date() } = {}) {
  ...
  function preview(payload = {}, actor) {
    ...
    return { candidate, before, resolved: after, revision: currentRevision, previewHash };
  }
