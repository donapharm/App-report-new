...
const COMPANY_LEGAL_NAME = 'CÔNG TY CỔ PHẦN DONAPHARM';
const COMPANY_TAX_CODE = '3603611886';
const CANONICAL_SIGNATURE_LINES = Object.freeze([
  'Trân trọng!',
  'CEO - ĐẶNG XUÂN TRUNG',
  COMPANY_LEGAL_NAME,
  `MST: ${COMPANY_TAX_CODE}`,
  'Hotline: 0886.396.668',
  'Email: info@donapharm.vn',
  'Email: trung.ceo@donapharm.vn',
  'Email này được gửi từ Văn phòng DONAPHARM với sự hỗ trợ của Trợ lý 🤖 Agent Donapharm.',
]);
...
function renderCanonicalSignatureHtml() {
  return `<div class='dona-signature' style='margin-top:28px;padding-top:16px;border-top:1px solid #d8eeee;font-size:13px;line-height:1.7;color:#294b4d;overflow-wrap:anywhere'>${CANONICAL_SIGNATURE_LINES.map((line, index) => `<div${index === 1 || index === 2 ? " style='font-weight:700'" : ''}${index === 7 ? " class='small' style='margin-top:6px'" : ''}>${esc(line)}</div>`).join('')}</div>`;
}
function renderCanonicalSignatureText() { return CANONICAL_SIGNATURE_LINES.join('\n'); }
