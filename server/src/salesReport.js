...
function defaultRanges(today = latestDataDate()) {
  ...
  return {
    ...
    prevRange: { from: startOfMonth(prevTo), to: prevTo },
    monthKy: kyOf(to),
    prevKy: kyOf(prevTo),
  };
}
...
function routeBreakdown(rows, prevRows) {
  const prev = new Map(groupRows(prevRows, 'route', 'route').map((x) => [x.key, x.revenue]));
...
function diffTop(curRows, prevRows, key, label, limit = 5) {
  ...
  const arr = [...keys].map((k) => ({ key: k, label: cur.get(k)?.label || prev.get(k)?.label || k, cur: cur.get(k)?.revenue || 0, prev: prev.get(k)?.revenue || 0 }));
...
async function computeReport({ empCode = 'DN001', kind = 'week', ranges = defaultRanges() } = {}) {
  ...
  const route = routeBreakdown(rows, prevRows);
  const diffsUnit = diffTop(rows, prevRows, 'unit_code', 'unit_name');
  const diffsProduct = diffTop(rows, prevRows, 'iit_code', 'product_name');
  ...
  return { kind, empCode, user, range, ranges, rows, prevRows, revenue, prevRevenue, score, topUnits, topProducts, route, diffsUnit, diffsProduct, cstRows, cstSource: cstPayload.source, target, forecast, days: dailyBars(rows) };
}
...
function renderHtml(data) {
  ...
  return `<!doctype html><html><head><meta charset='utf-8'><style>${css}</style></head><body><div style='padding:24px 0'><div class='wrap'><div class='banner'><table style='border:0;width:100%'><tr><td style='border:0;width:82px;vertical-align:top'><img src='cid:logo_dona' alt='DONAPHARM' style='width:64px;max-width:64px;height:auto;display:block;border:0;background:transparent'></td><td style='border:0;vertical-align:middle'><div class='brand'>DONAPHARM</div><div class='title'>Báo cáo doanh thu ${titleKind} — ${esc(data.user.name || data.empCode)}</div><div class='period'>${esc(data.user.name || data.empCode)} – ${esc(data.empCode)} | Kỳ ${data.range.from}–${data.range.to} | So sánh ${monthLabel(data.ranges.prevRange.to)}</div></td><td style='border:0;width:58px;text-align:right;vertical-align:top'><img src='cid:qr_zalo' alt='QR Zalo' style='width:46px;height:46px;display:block;margin:0 auto;border:0;background:transparent'></td></tr></table></div><div class='content'><p>Kính gửi <b>Anh/Chị ${esc(data.user.name || data.empCode)} (${esc(data.empCode)})</b>,</p><p>Văn phòng CEO gửi báo cáo doanh thu ${titleKind}, lọc riêng theo mã nhân viên <b>${esc(data.empCode)}</b>. Báo cáo dùng số live từ App Report, điểm/xu từ App VAT và CST từ App Sale.</p><p class='note'><b>Nguồn dữ liệu:</b> Doanh thu &amp; điểm từ <b>App Report-New</b> · Xu từ <b>App VAT (vat.db)</b> · CST từ <b>App Sale tender-quota</b>. Không chứa chi phí, giá vốn, lợi nhuận, margin. · <b>Xu chỉ tính theo QUÝ</b> — sang quý mới tự reset về 0, không chuyển tiếp.</p>
<div class='section'><h3>1. Tổng quan kết quả</h3><table class='overview-table'><tr><td><div class='kpi-card primary'><div class='label'>Doanh thu</div><div class='value'>${fmtMoney(data.revenue)}</div><div class='delta ${delta == null || delta >= 0 ? 'pos' : 'neg'}'>${delta == null ? 'Chưa có kỳ so sánh' : `${pct(delta)} so ${monthLabel(data.ranges.prevRange.to)}`}</div></div></td><td><div class='kpi-card'><div class='label'>Chênh lệch</div><div class='value ${data.revenue - data.prevRevenue >= 0 ? 'pos' : 'neg'}'>${fmtMoney(data.revenue - data.prevRevenue)}</div><div class='delta'>Kỳ trước: ${fmtMoney(data.prevRevenue)}</div></div></td></tr><tr><td><div class='kpi-card'><div class='label'>Số dòng</div><div class='value'>${data.rows.length}</div></div></td><td><div class='kpi-card'><div class='label'>Đơn vị</div><div class='value'>${unitCount}</div></div></td></tr><tr><td><div class='kpi-card'><div class='label'>Mặt hàng</div><div class='value'>${productCount}</div></div></td><td><div class='kpi-card'><div class='label'>Dự báo tháng</div><div class='value'>${fmtMoney(data.forecast)}</div></div></td></tr></table></div>
<div class='section'><h3>2. Điểm doanh thu &amp; xu chi tiêu</h3><table><tr><th>Kỳ</th><th class='r'>Doanh thu</th><th class='r'>Điểm DT tháng</th><th class='r'>Xu tuần</th><th class='r'>Xu tháng</th><th class='r'>Xu quý</th><th class='r'>Thiếu xu</th><th class='r'>Dư xu</th><th class='r'>Hoàn thành quý</th></tr><tr><td>${monthLabel(data.ranges.asOf)} đến ${data.ranges.asOf}</td><td class='r'>${fmtMoney(data.revenue)}</td><td class='r'><b>${fmtNum(data.score.diem_thang)}</b></td><td class='r'>${data.score.xu_tuan == null ? '—' : fmtNum(data.score.xu_tuan)}</td><td class='r'>${fmtNum(data.score.xu_thang)}</td><td class='r'>${fmtNum(data.score.xu_quy)}</td><td class='r neg'>${fmtNum(data.score.thieu_xu)}</td><td class='r pos'>${fmtNum(data.score.du_xu)}</td><td class='r'>${pct(data.score.ty_le_quy)}</td></tr></table></div>
<div class='section'><h3>3. Phân tích tuyến CL / NCL / NT</h3>${routeHtml || '<p>Chưa có dữ liệu tuyến.</p>'}<table><tr><th>Tuyến</th><th class='r'>Kỳ này</th><th class='r'>Tỷ trọng</th><th class='r'>Kỳ trước</th><th class='r'>Chênh lệch</th></tr>${htmlRows(data.route, [{ html: x => esc(x.key) }, { cls: 'r', html: x => fmtMoney(x.revenue) }, { cls: 'r', html: x => pct(x.pct) }, { cls: 'r', html: x => fmtMoney(x.prev) }, { cls: 'r', html: x => `<span class='${x.revenue - x.prev >= 0 ? 'pos' : 'neg'}'>${fmtMoney(x.revenue - x.prev)}</span>` }])}</table></div>
...
<div class='section'><h3>9. 🧠 Phân tích thông minh &amp; Định hướng bán hàng</h3><p class='highlight'>Phân tích tự động theo dữ liệu live: xu hướng so cùng kỳ tháng trước, dự báo cả tháng theo nhịp hiện tại, và hướng khai thác để cán target.</p><div class='grid2'><div class='kpi-card primary'><div class='label'>A. Xu hướng kỳ</div><div class='value ${delta == null || delta >= 0 ? 'pos' : 'neg'}'>${delta == null ? '—' : pct(delta)}</div><div class='delta'>So với ${monthLabel(data.ranges.prevRange.to)}.</div></div><div class='kpi-card' style='background:linear-gradient(180deg,#fff8eb,#fffdf7);border-color:#f0d9a0'><div class='label'>H. Dự báo cuối tháng theo nhịp</div><div class='value'>${fmtMoney(data.forecast)}</div><div class='delta'>Theo ${data.ranges.monthRange.from}–${data.ranges.monthRange.to}; target tháng: ${data.target ? fmtMoney(data.target) : 'chưa nhập'}.</div></div></div><div class='section' style='margin-top:14px'><h4>C. Cơ cấu tuyến &amp; cơ hội điểm</h4><p>NCL là dư địa mở rộng riêng, không phụ thuộc cơ số thầu. CL/NT và các đơn vị 025–028 giúp tăng điểm nhanh hơn do hệ số điểm ×2.</p></div><div class='section' style='margin-top:10px'><h4>I. Khuyến nghị khai thác để cán target</h4><table><tr><th>Hướng</th><th>Khuyến nghị cụ thể</th><th>Vì sao</th></tr><tr><td><b>⭐ NCL — dư địa vô hạn</b></td><td>Mở rộng đơn vị/nhà thuốc khối NCL — <b>không bị cơ số thầu chặn</b>.</td><td>Tăng doanh thu tự do để cán target</td></tr><tr><td>Khối CL (điểm ×2)</td><td>Ưu tiên đơn vị CL/025–028 đang có phát sinh và còn mã thầu dư.</td><td>Vừa doanh thu vừa điểm cao</td></tr><tr><td>Mã QLNB còn cơ số</td><td>${esc(cstText)}</td><td>Bán chắc, đã có thầu; số còn lại dùng thẳng slConLai App Sale</td></tr></table></div><p class='small'>Nguồn CST: ${esc(data.cstSource)}. Mục I lọc theo đơn vị NV có doanh thu/phụ trách; loại la_ap_thau khỏi danh sách hữu hạn.</p></div><p style='margin-top:28px'>Trân trọng,<br><b>ĐẶNG XUÂN TRUNG</b><br>CEO — Công ty TNHH Dược phẩm DONAPHARM<br>Hotline: 0886.396.668</p><p class='small'><em>E-mail này được gửi từ văn phòng CEO DONAPHARM với sự hỗ trợ AI agent Donapharm. Đây là bản test để CEO duyệt trước khi gửi chính thức cho nhân viên.</em></p></div></div></div></body></html>`;
}
function renderText(data) {
  const titleKind = data.kind === 'month' ? 'THÁNG' : 'TUẦN';
  const cst = data.cstRows.slice(0, 3).map((x) => `${x.productCode}@${x.unitCode}: ${fmtQty(x.slConLai)}`).join('; ') || 'chưa có mã CST khớp';
  return `DONAPHARM App Report — ${titleKind} ${data.empCode}\nKỳ ${data.range.from}–${data.range.to}\nDoanh thu: ${fmtMoney(data.revenue)} | Điểm tháng: ${fmtNum(data.score.diem_thang)} | Xu quý: ${fmtNum(data.score.xu_quy)} | Tỷ lệ: ${pct(data.score.ty_le_quy)}\nƯu tiên: giữ ${data.topUnits.slice(0, 2).map((x) => x.label).join(', ') || 'đơn vị chính'}; kéo lại ${data.diffsUnit.down.slice(0, 2).map((x) => x.label).join(', ') || 'đơn vị giảm'}; CST: ${cst}.`;
}
async function renderEmployeeReport(opts) {
  const data = await computeReport(opts);
  return { data, html: renderHtml(data), text: renderText(data), subject: `DONAPHARM App Report — Báo cáo ${data.kind === 'month' ? 'tháng' : 'tuần'} ${data.empCode} (${data.range.from}–${data.range.to})` };
}
async function renderCeoDigest({ kind = 'week', ranges = defaultRanges() } = {}) {
  ...
  return { html, text, subject: `DONAPHARM App Report — CEO Digest ${kind === 'month' ? 'tháng' : 'tuần'} (${ranges.monthRange.from}–${ranges.monthRange.to})`, rows };
}
...
async function main() {
  ...
  if (cmd === 'sample') {
    ...
    console.log(JSON.stringify({ files: r.files, ceoEmail: r.ceoEmail, sendResults: r.sendResults, ranges: r.ranges, cstSource: r.week.data.cstSource, cstRows: r.week.data.cstRows.length }, null, 2));
    return;
  }
