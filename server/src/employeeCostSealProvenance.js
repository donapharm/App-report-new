...
const EXPECTED_SOURCE_CAPTURE_COUNT = 21;
...
function buildEnvelope(reports, { observedAt = new Date() } = {}) {
  ...
  if (captured.length !== EXPECTED_SOURCE_CAPTURE_COUNT) return null;
  if (!captured.every(validCaptured)) return null;
  if (!captured.every((item) => sameDeclaration(item, captured[0]))) return null;
  const appReportResponseRowCount = captured.reduce((sum, item) => sum + item.appReportResponseRowCount, 0);
  if (!Number.isSafeInteger(appReportResponseRowCount)
    || appReportResponseRowCount !== captured[0].c32SidecarRowCount) return null;
