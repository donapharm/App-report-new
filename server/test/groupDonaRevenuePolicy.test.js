...
test('T09 onward Group-Dona accepts only invoiced Debts and rejects CRM without touching partners', () => {
  const rows = [
  ...
  assert.deepEqual(result.accepted, [rows[0], rows[3]]);
  assert.deepEqual(result.rejected, [rows[1], rows[2]]);
});
