...
test('runner tôn trọng hai luật cứng của V-D', () => {
  ...
  // 2. payment_notice chỉ được đăng ký sau công tắc chủ mặc định OFF.
  assert.match(SOURCE, /if \(services\.paymentNoticeEnabled\(\)\) handlers\.payment_notice = paymentNotice/);
...
test('--payment-dry-run khi công tắc OFF không dựng sổ, không gửi và không ghi', () => {
  const out = execFileSync(process.execPath, [SCRIPT, '--payment-dry-run'], {
    encoding: 'utf8', env: { ...process.env, PAYMENT_NOTICE_ENABLED: '' },
  });
  assert.match(out, /PAYMENT DRY-RUN/);
  assert.match(out, /công tắc chủ đang OFF/);
  assert.match(out, /gửi 0, ghi 0/);
});
