...
async function checkKind(kind) {
  const data = await deckData.build({ kind });
  const html = deckHtml.render(data);
  ...
  const stemPrefix = kind === 'week' ? 'BAO_CAO_DOANH_SO_TUAN_' : 'BAO_CAO_DOANH_SO_THANG_';
  const files = fs.readdirSync(deckReport.OUT_DIR).filter((x) => x.startsWith(stemPrefix) && x.includes('_DRAFT'));
  const htmlName = files.find((x) => x.endsWith('.html'));
  const pptxName = files.find((x) => x.endsWith('.pptx'));
  const manifestName = files.find((x) => x.endsWith('.manifest.json'));
  assert.ok(htmlName && pptxName && manifestName, `Missing built ${kind} artifacts`);
  const htmlPath = path.join(deckReport.OUT_DIR, htmlName), pptxPath = path.join(deckReport.OUT_DIR, pptxName);
  const manifest = JSON.parse(fs.readFileSync(path.join(deckReport.OUT_DIR, manifestName)));
