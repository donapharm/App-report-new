...
### 2026-08-21 — App Report Dev Bot — NHỊP 1 dời kho phiên PASS 4/4 (read-only acceptance)

- **Phạm vi:** chỉ dời kho phiên qua `AUTH_DATA_DIR`; production giữ nguyên exact `011ac1e7fab6151a15e1463a2b1c55d2de86656b`. Không đổi nguồn doanh thu, không sửa/xóa dữ liệu và không chạy Face ID.
- **Luật nghiệm thu số dòng:** kỳ đang mở so cùng thời điểm trước/sau; kỳ đã đóng dùng baseline cố định. T08 trước/sau cutover đều `1.481 dòng / 19 NV` (tổng kho `1.492`, ngoài đội hình `11`); snapshot nguồn dùng để đối chiếu không đổi qua cutover. T07 hiện tại `2.087 dòng / 19 NV`, khớp baseline đóng kỳ.
- **Mục tiêu dời kho phiên PASS:** CEO đăng nhập OTP, `app-report` restart đúng một lần và trình duyệt vẫn còn phiên, không phải đăng nhập lại. Kho phiên giữ nguyên `27 → 27` qua restart.
- **Runtime PASS:** `/api/health` local/public đều xanh và cùng version `011ac1e-20260820-164657-080`; `app-report` và `app-report-tgbot` online; cửa sổ log 5 phút sau cutover không có lỗi mới.
- **T08 ngoài đội hình (read-only):** `11` dòng, tổng doanh thu trước VAT `294.328.300đ`. Trong đó `4` dòng mã `DN021`, `6` dòng mã `DN023` không thuộc roster 19 NV hiện tại; `1` dòng nguồn `VP018` được chuẩn hóa thành `UNALLOCATED`, không khớp roster. Chỉ ghi nhận để CEO đối soát trước khóa sổ; không gán lại NV.
- **Rollback:** không kích hoạt vì không có mất dữ liệu, lộ quyền hoặc lỗi boot. Bản sao lưu trước cutover và release `stack-ab-011ac1e` tiếp tục được giữ nguyên.
- **Face ID:** vẫn khóa; cấm deploy `43cdbad` nguyên trạng. Candidate phải rebase lên `011ac1e`, test, push exact mới và được Claude review ĐẠT trước cổng deploy riêng.
