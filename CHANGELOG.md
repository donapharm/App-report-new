### 2026-07-21 — Report Bot — Chi phí timeline: danh sách dòng do doanh thu App Report dẫn dắt
- Đảo `enrichWithRevenue` sang **revenue-driven**: mỗi cặp `c7 đơn vị + mã hàng` DN001/NV thực bán trong đúng tháng tạo đúng một dòng; dòng chỉ có trong DataHub cost không còn xuất hiện. Phần trăm DataHub chỉ là lookup theo kỳ/timeline, không còn quyết định danh sách dòng.
- Lookup vẫn fail-closed: `c16/c5` phải resolve qua canonical catalog rồi mới ghép bằng mã; không join tên thô. Cost thiếu %, không có dòng hoặc trùng nhiều dòng cho cùng khóa vẫn giữ dòng doanh thu với tỷ lệ/Thành tiền `—`, `revenueMatched=false` và tính vào mẫu số tỷ lệ chưa khớp.
- Giữ tên hàng/ĐVT từ canonical catalog (fallback doanh thu), công thức `round(doanh thu × % / 100)`, c44 tách cuối năm, đối soát `Σ ngày = tháng`, self-scope, khóa C32/C47 và visibility backend. Bổ sung regression T07 Cerecaps 3 khóa doanh thu, cost-only bị loại, missing/duplicate cost, timeline T06→T07 đổi mức và daily rounding.
- Bổ sung test revenue-driven/timeline/carry và giữ toàn bộ test visibility. Proxy nhận đúng cặp env đang deploy `DATA_HUB_BASE_URL`/`DATA_HUB_ASSIGNMENT_KEY` (vẫn tương thích alias cũ), khắc phục trạng thái rỗng do cấu hình không được đọc.
- QA code: `employeeCost` + `employeeCostVisibility` **31/31 test PASS**. Live read-only trên snapshot App Report T07 lúc 17:30: DN001 có **170 khóa doanh thu**, khớp **158/170 = 92,9%**; snapshot hiện tại chỉ có **1** khóa Cerecaps DN001 (`038.PKĐK THIỆN NHÂN-CN2`, doanh thu 7.980.000đ), nên nghiệm thu số thật “3 dòng Cerecaps” còn chờ App Report nạp đủ 3 khóa doanh thu kỳ CEO nêu. Thay đổi chỉ nằm trên nhánh review, **chưa deploy**.
