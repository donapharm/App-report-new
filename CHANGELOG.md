### 2026-07-22 — Claude Code (giao bot) — SỬA "Lỗi máy chủ" trang Chi phí (500 ở /employee-cost/visibility)
- **CEO báo trang Chi phí hiện "Lỗi máy chủ" + "Chưa có nhân viên" + bảng trống.** Chẩn đoán: **1 lỗi backend lan ra 3 chỗ** — endpoint ADMIN `/employee-cost/visibility` (cũng nạp picker NV) **crash 500**.
- **Chỗ văng:** `employeeCostRoster.loadConfig()` đọc `config/employee_cost_groups.json` bằng `JSON.parse(fs.readFileSync(...))` **không try/catch** → file thiếu/`EMPLOYEE_COST_GROUP_CONFIG` trỏ sai/JSON hỏng ⇒ throw ⇒ 500 trơn ⇒ FE hiện "Lỗi máy chủ" chung. 2 route GET (`visibility`/`employees`) **không bọc bắt lỗi** như route POST nên lý do thật không lọt ra. (`/me` login ADMIN short-circuit nên chỉ mở trang chi phí mới lộ.)
- **Sửa (directive `DIRECTIVE_EMP_COST_VISIBILITY_500_FIX.md`, cùng nhánh review):** (1) `loadConfig` try/catch → fallback `{}` (suy biến mượt, không sập trang); (2) bọc 2 route GET trả `{error}` cụ thể. Lỗi độ-bền-cấu-hình, không đụng số/quyền/VAT/grain. Chưa deploy.
