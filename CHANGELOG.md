...
- **LIVE parity còn [blocked] an toàn:** chưa gửi service token thật vì App VAT hiện log prefix Bearer tại `webapp_vat/server.js:1260` (và nhánh validate tại `:2144`). Cần App VAT bỏ log token + cấu hình secret backend rồi mới chạy đối chiếu cùng NV/kỳ; không hạ tiêu chuẩn bảo mật để lấy bằng chứng. Không deploy/restart.
