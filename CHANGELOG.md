### 2026-07-22 — Claude Code (giao bot) — "Lỗi máy chủ" trang Chi phí: nguyên nhân THẬT = process cũ (404), không phải config
- **CEO báo trang Chi phí hiện "Lỗi máy chủ" + "Chưa có nhân viên" + bảng trống.** Chẩn đoán ban đầu của Claude (loadConfig 500) **SAI cho lần này** — bot kiểm tra: config OK, `loadConfig()` OK, **không có stack**; `curl` trả **404**, không phải 500.
- **Nguyên nhân thật:** **process production khởi động 11:59, trước khi route `/employee-cost/visibility` thêm lúc 12:05** → BE chưa nạp route mới; **FE bản mới** gọi route đó → **404** → FE map mọi mã ≠ 2xx về câu chung "Lỗi máy chủ". Tức **lệch phiên bản FE mới / BE cũ**, KHÔNG phải lỗi code.
- **Fix vụ này = restart/deploy** để BE nạp route (gộp vào quyết định deploy lõi). Directive `DIRECTIVE_EMP_COST_VISIBILITY_500_FIX.md` cập nhật: **loadConfig hardening hạ xuống phòng-vệ-tùy-chọn** (vẫn nên có, chống 500 thật nếu tương lai mất config); giữ bọc route GET trả `{error}`; polish FE phân biệt 404 vs 500 (ưu tiên thấp). Chưa deploy.
