...
const page = pages.find(p => p.url?.includes('report.donapharm.asia') && p.type === 'page') || pages.find(p => p.type === 'page');
if (!page?.webSocketDebuggerUrl) throw new Error('Không tìm thấy tab App Report/CDP ws');
