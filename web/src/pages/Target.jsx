...
export default function Target({ me }) {
  ...
  useEffect(() => { api.targets().then(setNow); }, []);
            ...
            return (
              <div key={t.emp_code} className="card" style={{ padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ fontWeight: 700 }}>{t.emp_name || t.emp_code}</div>
            ...
            {fc.items.map((t) => (
              <div key={t.emp_code} className="card" style={{ padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <div style={{ fontWeight: 700 }}>{t.emp_name}</div>
                  <div style={{ fontWeight: 800, color: 'var(--brand)' }}>{short(t.suggested_target)}</div>
                </div>
