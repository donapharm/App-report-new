...
function TargetAdminPanel({ ky, onKyChange, onTargetsChanged }) {
  ...
  async function commitUpload() {
    ...
    setBusy(true); setErr(''); setMsg('');
    try { const r = await api.adminTargetUploadCommit(preview.previewId); setLastBatch(r.result); setMsg(`Đã import ${r.result.rows} dòng target. Mã rollback: ${r.result.batchId}. KPI Tổng đã cập nhật.`); setPreview(null); if (fileRef.current) fileRef.current.value = ''; await load(); await onTargetsChanged?.(); }
    catch (e) { setErr(e.message); }
    setBusy(false);
  }
  async function rollbackBatch(batchId) {
    ...
    setBusy(true); setErr(''); setMsg('');
    try { const r = await api.adminTargetUploadRollback(id); setMsg(`Đã rollback ${r.result.rows} dòng target của batch ${id}. KPI Tổng đã cập nhật.`); if (lastBatch?.batchId === id) setLastBatch(null); await load(); await onTargetsChanged?.(); }
    catch (e) { setErr(e.message); }
    setBusy(false);
  }
  ...
  return (
    <>
      <div className="card">
        ...
        <div className="meta muted">Resolver đang dùng: sửa tay &gt; upload &gt; AI; từ kỳ 07.2026 trở đi không lấy Lumos/App Sale làm target đang dùng. Roster Target lấy theo allowlist CEO chốt 21 mã.</div>
        <div className="target-admin-actions">
          <label className="field-inline"><span>Kỳ target</span><input value={ky || ''} onChange={(e) => onKyChange?.(e.target.value)} placeholder="08.2026" /></label>
        ...
        <div className="target-admin-actions">
          ...
          <label className="field-inline"><span>Căn cứ template</span><select value={templateBasis} onChange={(e) => setTemplateBasis(e.target.value)}><option value="t06">Theo T06/2026 (Lumos)</option><option value="blank">Trống</option><option value="latest">Theo kỳ gần nhất đã giao</option></select></label>
          <button className="btn" disabled={busy} onClick={downloadTemplate}>⬇ Xuất/Tải template target</button>
          <button className="btn ghost" disabled={busy} onClick={() => rollbackBatch()}>↩ Rollback theo mã</button>
        ...
        <div className="meta muted">Template xuất đúng kỳ đang chọn, đủ 21 NV theo DB. Nếu kỳ tương lai chưa giao target, mặc định điền sẵn target T06/2026 Lumos làm căn cứ để CEO sửa rồi upload; số căn cứ không tự thành target live. Khi upload lại: ô Target trống sẽ giữ nguyên, không ghi đè target hiện tại. KPI Tổng cập nhật ngay sau sửa/commit/rollback.</div>
      </div>
      <div className="card">
        <div className="section-head">📅 Nhập target Quý — chia đều 3 tháng</div>
      ...
      {lastBatch?.batchId && <div className="card" style={{ borderColor: 'var(--warn)', color: 'var(--text)' }}>
        ...
        <button className="btn ghost" disabled={busy} onClick={() => rollbackBatch(lastBatch.batchId)}>↩ Rollback batch này</button>
      </div>}
...
export default function Target({ me }) {
  ...
  return (
    <>
      ...
      {view === 'admin' && me.isAdmin && periodSel && <PeriodFilter periods={periods} value={periodSel} onChange={(sel) => { setPeriodSel(sel); setAdminKy(sel.mode === 'range' ? sel.to : sel.ky); }} />}
