...
const EMPTY_OVERVIEW_FILTERS = { emp: '', route: '', companyGroup: '', unitGroup: '', unit: '' };
...
function OverviewFilters({ me, filters, setFilters, options, busy }) {
  ...
  const setF = (key, value) => setFilters((current) => {
    ...
    if (key === 'unitGroup' && value !== current.unitGroup) next.unit = '';
    return next;
  });
...
export default function Overview({ me, onNavigate }) {
  ...
  const overviewParams = useMemo(() => periodSel
    ? { ...periodParams(periodSel), ...activeOverviewFilters(overviewFilters) }
    : null, [periodSel, overviewFilters]);
  const targetComparable = !overviewFilters.route && !overviewFilters.companyGroup && !overviewFilters.unitGroup && !overviewFilters.unit;
  ...
  function loadTopRows(p) {
    ...
    if (topDim === 'emp' && me.isAdmin) {
      Promise.all([api.revenue('emp', null, p), api.targets(p)]).then(([d, t]) => {
        ...
        setTopRows((d.rows || []).slice(0, topLimit).map((r) => ({ ...r, pctTarget: targetComparable ? (pctByEmp[r.key] ?? null) : null })));
      });
    } else {
  ...
  useEffect(() => {
    ...
    let active = true;
    setOverviewFilterBusy(true);
    api.filters(overviewParams).then((next) => {
      if (!active) return;
      setOverviewOptions(next);
      setOverviewFilters((current) => {
        const out = { ...current };
        const specs = [
          ['emp', 'employees', true], ['route', 'routes', true], ['companyGroup', 'companyGroups', false],
          ['unitGroup', 'unitGroups', false], ['unit', 'units', false],
        ];
        for (const [field, optionKey, multi] of specs) {
          const allowed = new Set((next?.[optionKey] || []).map((item) => String(item.key)));
          if (multi) out[field] = String(out[field] || '').split('|').filter((value) => allowed.has(value)).join('|');
          else if (out[field] && !allowed.has(String(out[field]))) out[field] = '';
        }
        return JSON.stringify(out) === JSON.stringify(current) ? current : out;
      });
    }).finally(() => { if (active) setOverviewFilterBusy(false); });
    return () => { active = false; };
  }, [overviewParams]);
  ...
  }, [overviewParams, topDim, reloadTick]);
  ...
  useEffect(() => {
    ...
    api.revenue('unit', null, overviewParams).then((d) => {
      if (active) setUnitRevenueRows(d.rows || []);
    });
    return () => { active = false; };
  }, [overviewParams, reloadTick]);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      api.analysis({ ...overviewParams, compareMode: cmpMode }),
  ...
  const nearTarget = (targetComparable ? targetItems : [])
  ...
  const targetSlowItems = !targetComparable ? [] : targetSlowUsesPacing
  ...
  const targetSlowTotal = !targetComparable ? 0 : targetSlowUsesPacing ? Number(targetSlowGroup.total || 0) : targetSlowItems.length;
  const targetSlowNote = !targetComparable
    ? 'Target chưa được phân bổ theo lát cắt tuyến/group/nhóm đơn vị.'
    : targetSlowUsesPacing && targetPacing?.isCurrent
  ...
  async function refreshNow() {
    ...
    try {
      ...
      loadTopRows(overviewParams);
      setUnitRevenueRows(null);
      api.revenue('unit', null, overviewParams).then((d) => setUnitRevenueRows(d.rows || []));
      setRichInsights(null);
      Promise.allSettled([
        api.analysis({ ...overviewParams, compareMode: cmpMode }),
        api.targets(overviewParams),
    ...
    <>
      ...
      {periodSel && (
        <div className="muted" style={{ display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'flex-end', margin: '-6px 0 10px' }}>
          <span>Dữ liệu đến ngày {dataAsOfText || '—'} · {syncAndActivationSame ? `Job chạy và slot kích hoạt cùng lúc ${formatDateTime(syncAt)}` : `Job chạy lúc ${syncAt ? formatDateTime(syncAt) : '—'} · Slot kích hoạt lúc ${activatedAt ? formatDateTime(activatedAt) : '—'}`} (GMT+7) · Nguồn {sourceName}{partitionFreshness} · {lastSyncText}</span>
        ...
        <>
          ...
          <div className="chart-grid overview-charts">
            <div className="card chart-card wide">
              <div className="section-head">📈 Doanh thu theo kỳ · {kpi.targetComparable === false ? 'target không phân bổ theo lát cắt' : 'overlay target'}</div>
            ...
            <div className="card chart-card target-card">
              ...
              {kpi.targetComparable === false ? (
                <div className="overview-target-unavailable"><b>Không tính % target theo lát cắt</b><span>Target hiện được giao theo nhân viên, chưa phân bổ theo tuyến, Group hoặc nhóm đơn vị.</span></div>
              ) : <TargetGauge pct={kpi.pctTarget} />}
              {kpi.targetComparable === false ? null : Number(kpi.targetTotal || 0) > 0 ? (
