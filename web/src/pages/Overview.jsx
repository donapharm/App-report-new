...
export default function Overview({ me, onNavigate }) {
  ...
  useEffect(() => {
    if (!periodSel) return;
    const dim = topDim === 'emp' && !me.isAdmin ? 'unit' : topDim;
    if (dim !== topDim) { setTopDim(dim); return; }
    setTopRows(null);
    api.revenue(dim, null, periodParams(periodSel)).then((d) => setTopRows((d.rows || []).slice(0, topLimit)));
  }, [periodSel, topDim, reloadTick, me.isAdmin]);
        ...
        <>
          ...
          <div className="chart-grid overview-charts">
            ...
            <div className="card chart-card wide">
              ...
              {!topRows ? <Spinner /> : <TopBarChart rows={topRows} limit={topLimit} dimension={topDim} totalRevenue={kpi.revenue} />}
            </div>
          </div>
        </>
      )}
