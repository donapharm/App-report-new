...
function normalizedMatch(rawMatch = {}, rowCount = 0) {
  const safeReasons = Object.fromEntries(Object.entries(rawMatch.unavailableReasons || {})
    .map(([emp, reason]) => [String(emp), ['rate_policy_missing', 'rate_policy_unavailable', 'rate_policy_ambiguous', 'rate_policy_not_applicable', 'not_configured', 'upstream_unavailable', 'upstream_rejected', 'upstream_busy', 'deadline', 'source_error', 'missing_snapshot'].includes(String(reason))
  ...
  return chanTongToanDoi({
    ...
    trangThaiDongBo: payload.trangThaiDongBo && typeof payload.trangThaiDongBo === 'object' ? {
      ...
      unavailableReasons: Object.fromEntries(Object.entries(payload.trangThaiDongBo.unavailableReasons || {})
        .map(([emp, reason]) => [String(emp), ['rate_policy_missing', 'rate_policy_unavailable', 'rate_policy_ambiguous', 'rate_policy_not_applicable', 'not_configured', 'upstream_unavailable', 'upstream_rejected', 'upstream_busy', 'deadline', 'source_error', 'missing', 'roster_added', 'roster_changed', 'corrupt_snapshot', 'sync_failed', 'locked'].includes(String(reason))
          ? String(reason) : 'upstream_unavailable'])),
    ...
    revenueRecon: payload.revenueRecon && typeof payload.revenueRecon === 'object' ? {
      ...
      outsideRosterCodes: (Array.isArray(payload.revenueRecon.outsideRosterCodes)
        ? payload.revenueRecon.outsideRosterCodes : []).map(String),
