...
function initialForm(items = []) {
  return Object.fromEntries(items.map((item) => {
    const a = item.action || item.current_action || {};
    return [itemId(item), { status: a.status || '', next_follow_up: a.next_follow_up || '', note: a.note || '' }];
  }));
}
function complete(value = {}, today = '') {
  if (!value.status) return false;
  if (NEED_DATE.has(value.status) && (!value.next_follow_up || (today && value.next_follow_up <= today))) return false;
...
function DormantItem({ item, value, onChange, index, minFollowUp }) {
  ...
  return <article className="dormant-item">
    ...
    <div className="dormant-unit">🏥 {item.unit_name || item.unit_code || '—'} <small>· {item.route || '—'}</small></div>
    ...
    <div className="dormant-action-grid">
      ...
      <label><span>Ngày theo dõi lại{NEED_DATE.has(value.status) ? ' *' : ''}</span><input type="date" min={minFollowUp} value={value.next_follow_up} onChange={(e) => onChange({ ...value, next_follow_up: e.target.value })} /></label>
...
export default function DormantGate({ me, tab }) {
  ...
  useEffect(() => {
    ...
    api.dormantGate({ source: tab }).then((result) => {
      if (cancelled) return;
      setData(result);
      setForm(initialForm(result.required_items || result.items || []));
    }).catch((e) => { if (!cancelled) setError(e.message); });
    return () => { cancelled = true; };
  }, [eligible, tab]);

  const ready = useMemo(() => items.length > 0 && items.every((item) => complete(form[itemId(item)], data?.generated_on)), [items, form, data?.generated_on]);
  ...
  async function submit() {
    ...
    try {
      ...
      setData(next);
      setForm(initialForm(next.required_items || next.items || []));
  ...
  return <div className="dormant-gate-backdrop" role="dialog" aria-modal="true" aria-labelledby="dormant-gate-title">
    <div className="dormant-gate-card">
      <header className="dormant-gate-head">
        ...
        <div className="dormant-count"><b>{Number(data.summary?.dormant_total || items.length).toLocaleString('vi-VN')}</b><small>mã cần theo dõi</small></div>
      </header>
      <div className="dormant-gate-body">
        <XuStrip data={data.xu} />
        <div className="dormant-intro"><b>Vui lòng cập nhật {items.length} mã ưu tiên trước khi xem báo cáo.</b><span>Hệ thống chỉ hỏi mã mới ngủ đông hoặc đã đến hạn chăm sóc; mã đã có kế hoạch và chưa đến hạn sẽ không hỏi lại.</span></div>
        {items.map((item, index) => <DormantItem key={itemId(item)} item={item} index={index} minFollowUp={minFollowUp} value={form[itemId(item)] || { status: '', next_follow_up: '', note: '' }} onChange={(value) => setForm((old) => ({ ...old, [itemId(item)]: value }))} />)}
        {error && <div className="dormant-error">{error}</div>}
      </div>
      <footer className="dormant-gate-foot">
        <span>🔒 Phản hồi được lưu theo tài khoản và thời điểm để theo dõi đến khi có đơn trở lại.</span>
        <button type="button" className="btn" disabled={!ready || busy} onClick={submit}>{busy ? 'Đang lưu…' : 'Xác nhận kế hoạch và xem báo cáo'}</button>
      </footer>
    </div>
  </div>;
}
