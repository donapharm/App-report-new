/* dona-table-cell-tools v1.0.1 | dependency-free browser IIFE */
(function (global) {
'use strict';
...
const LINE_OPTIONS = Object.freeze([1, 2, 3, 'all']);
...
function normalizeLines(value, fallback = DEFAULT_LINES) {
  if (value === 'all') return 'all';
  const numeric = Number(value);
  return numeric === 1 || numeric === 2 || numeric === 3 ? numeric : fallback;
}

function storageKey(appId, tableId) {
...
function resolveIdentity(root, options = {}) {
...
function getMounted(root) { return INSTANCES.get(root) || null; }
function destroy(root) { const instance = INSTANCES.get(root); if (!instance) return false; instance.destroy(); return true; }


global.DonaTableCellTools = Object.freeze({ VERSION, DEFAULT_LINES, LINE_OPTIONS, DEFAULT_EXCLUDE_SELECTOR, DEFAULT_INTERACTIVE_SELECTOR, normalizeLines, storageKey, readStoredLines, writeStoredLines, isMountableRoot, isInteractiveTarget, canActivateCell, getCellValue, resolveIdentity, mount, getMounted, destroy });
