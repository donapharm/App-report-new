...
Deploy app lên **https://report.donapharm.asia** theo mô hình **1 server Node (:3873) + Cloudflare Tunnel** (app phục vụ cả giao diện lẫn API trên cùng cổng; không mở port ra ngoài).
...
- URL `https://report.donapharm.asia` đã sống chưa? Kết quả `curl /api/health`, `pm2 status`, `cloudflared` status.
