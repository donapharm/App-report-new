...
- Worker: gắn vào tiến trình `app-report-tgbot` (hoặc cron riêng). Múi giờ **Asia/Bangkok**.
...
- **Claude đã sửa text code** (targetNotify.js, notifyChannels.js, routes.js, web/src). **Bot còn phải đổi ENV
  `SMTP_FROM`** trong `.env` → `SMTP_FROM="DONAPHARM App Report <...>"` rồi `pm2 restart app-report --update-env`
...
- Gắn vào tiến trình worker (`app-report-tgbot`) hoặc cron riêng; log mỗi lần gửi (số gửi/skip/lỗi).
...
- **ĐÃ THÔNG SMTP (2026-07-09):** gửi thử `{ok:true}`. Còn lại: nv_emails.json đầy đủ + restart process app-report
