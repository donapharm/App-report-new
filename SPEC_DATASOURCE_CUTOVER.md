...
2. **Từ 07/2026:** kéo từ **2 nguồn App Sale ecosystem** theo từng kỳ: CRM MISA snapshot/read-model (`misa_revenue_snapshot_lines`, đã xuất HĐ) + APP WEB đối tác đã giao thực (`partner_order_line_responses`). Materialize mỗi tháng đã đóng thành JSON (như slot) để nhẹ + ổn định; tháng hiện tại refresh định kỳ. Không dùng WEB ordered làm doanh thu thực. **Quy tắc gán kỳ:** CRM MISA theo `revenue_date`/ngày xuất HĐ; APP WEB Partner theo **kỳ đơn đặt `orders.created_at` (Asia/Bangkok)** rồi mới xét đã giao đủ/đã thực hiện. Đơn chuyển kỳ không kéo sang kỳ sau chỉ vì ngày phản hồi/giao nằm kỳ sau (trace T07 loại `DT-260630-0115`, `1.960.000đ`). Kỳ đã khóa chỉ điều chỉnh bằng adjustment/carryover có duyệt.
